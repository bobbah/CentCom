...
import { Gavel, UserCheck, Alarm, Confetti, X } from 'tabler-icons-react';
    ...
    <Timeline active={ban.active ? 0 : 1} bulletSize={24} lineWidth={2} color={color}>
      ...
      {ban.expires !== undefined ? (
        (ban.unbannedBy !== undefined && (
          <Timeline.Item bullet={<UserCheck size={16} />} title="Unbanned">
            <Text color="dimmed" size="sm">
              Ban was lifted by{' '}
              <Text span fw={400} c={color} inherit>
                {ban.unbannedBy}
          ...
          <Timeline.Item
            bullet={ban.active ? <Alarm size={16} /> : <Confetti size={16} />}
        ...
        <Timeline.Item bullet={<X color="red" size={16} />} title="Permanent">
          <Text color="dimmed" size="sm">
